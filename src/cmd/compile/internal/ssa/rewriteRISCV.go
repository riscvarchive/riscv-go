...
func rewriteValueRISCV(v *Value, config *Config) bool {
	...
	case OpITab:
		return rewriteValueRISCV_OpITab(v, config)
	case OpInterCall:
...
func rewriteValueRISCV_OpClosureCall(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (ClosureCall entry _ mem)
	// cond:
	// result: (ADD entry mem)
	for {
		entry := v.Args[0]
		mem := v.Args[2]
		v.reset(OpRISCVADD)
...
func rewriteValueRISCV_OpITab(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (ITab)
	// cond:
...
func rewriteValueRISCV_OpStaticCall(v *Value, config *Config) bool {
	b := v.Block
	_ = b
	// match: (StaticCall [argwid] {target} _)
	// cond:
	// result: (ADD [argwid] {target})
	for {
		argwid := v.AuxInt
		target := v.Aux
		v.reset(OpRISCVADD)
