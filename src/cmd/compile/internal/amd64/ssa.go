...
func ssaGenValue(s *gc.SSAGenState, v *ssa.Value) {
	...
	case ssa.OpAMD64ADDB, ssa.OpAMD64ADDSS, ssa.OpAMD64ADDSD,
		...
		if x != r && y != r {
			gc.OpRegReg(moveByType(v.Type), r, x)
			x = r
		}
		p := gc.Prog(v.Op.Asm())
		p.From.Type = obj.TYPE_REG
	...
	case ssa.OpAMD64SUBSS, ssa.OpAMD64SUBSD, ssa.OpAMD64DIVSS, ssa.OpAMD64DIVSD:
		...
		if y == r && x != r {
			...
			// rename y with x15
			gc.OpRegReg(moveByType(v.Type), x15, y)
			gc.OpRegReg(moveByType(v.Type), r, x)
			y = x15
		} else if x != r {
			gc.OpRegReg(moveByType(v.Type), r, x)
	...
	case ssa.OpAMD64CMPQ, ssa.OpAMD64CMPL, ssa.OpAMD64CMPW, ssa.OpAMD64CMPB,
		...
		gc.OpRegReg(v.Op.Asm(), gc.SSARegNum(v.Args[1]), gc.SSARegNum(v.Args[0]))
	case ssa.OpAMD64UCOMISS, ssa.OpAMD64UCOMISD:
		// Go assembler has swapped operands for UCOMISx relative to CMP,
		// must account for that right here.
		gc.OpRegReg(v.Op.Asm(), gc.SSARegNum(v.Args[0]), gc.SSARegNum(v.Args[1]))
	...
	case ssa.OpAMD64MOVLQSX, ssa.OpAMD64MOVWQSX, ssa.OpAMD64MOVBQSX, ssa.OpAMD64MOVLQZX, ssa.OpAMD64MOVWQZX, ssa.OpAMD64MOVBQZX,
		...
		gc.OpRegReg(v.Op.Asm(), gc.SSARegNum(v), gc.SSARegNum(v.Args[0]))
	...
	case ssa.OpCopy, ssa.OpAMD64MOVQconvert: // TODO: use MOVQreg for reg->reg copies instead of OpCopy?
		...
		y := gc.SSARegNum(v)
		if x != y {
			gc.OpRegReg(moveByType(v.Type), y, x)
		}
	case ssa.OpLoadReg:
		if v.Type.IsFlags() {
	...
	case ssa.OpAMD64SETNEF:
		...
		gc.OpRegReg(x86.AORL, gc.SSARegNum(v), x86.REG_AX)
	...
	case ssa.OpAMD64SETEQF:
		...
		gc.OpRegReg(x86.AANDL, gc.SSARegNum(v), x86.REG_AX)
